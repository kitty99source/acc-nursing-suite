...
**Success:** `04f092d` — full WebSocket CDP harvest, 45 SSRS links, base URL `http://internal-portal-server/Reports_MSREPORT/`.
...
| **Real email format recognized** | ✅ | From `Sample.Manager@example.test`; subject pattern **`Claim:` + `ACCID:`** (e.g. `Mr Gilbert Gandor - Claim:10000003194 ACCID:VEND-K96655`) |
...
11. **Email filter** — senders: `Sample.Staffer@example.test`, `Sample.Manager@example.test`, `Sample.Approver@example.test`; subject must contain **`Claim:`** AND **`ACCID:`**; PDF/DOCX attachments.
...
- Real email format: Sample.Manager@example.test, subject Claim: + ACCID:
...
2. Confirm sender `Sample.Manager@example.test` in allowlist
