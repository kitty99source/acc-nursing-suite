...
**Portal note:** Internal URL `http://internal-portal-server/...` is reachable only on hospital network or Citrix VPN. Automation runs on the **work PC**, not from Cursor cloud agents. Do **not** automate Citrix VPN login with stored passwords — manual VPN + manual portal SSO per session, then CDP attach.
...
Internal portal URL (`http://internal-portal-server/...`) is **not reachable** from Cursor cloud or home dev machines without Citrix VPN. Full blind scrape without login is **not possible** from here.
...
| Condition | Value |
|-----------|-------|
| **From** | `Sample.Staffer@example.test` OR `Sample.Manager@example.test` OR `Sample.Approver@example.test` |
