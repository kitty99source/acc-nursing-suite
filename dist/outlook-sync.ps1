        ...
        try {
            ...
            for ($i = 1; $i -le $attachments.Count; $i++) {
                ...
                try {
                    ...
                    # Embed patient + claim (parsed from the subject) INTO the saved name so the
                    # reviewer can identify the letter without opening it. Bytes are unchanged, so
                    # the folder-watch content SHA-256 dedup is unaffected. Get-UniquePath still
                    # uniquifies on-disk collisions of the descriptive name.
                    $descriptiveName = New-DescriptiveFileName -Subject $subject -OriginalFileName $fileName
                    $dest = Get-UniquePath -Dir $inbox -FileName $descriptiveName
                    $att.SaveAsFile($dest)
                    $savedAny = $true
                    $status.savedCount++
                    $status.savedFiles += @{
