...
describe('accInboxFilters — editable Settings rules (P8-018)', () => {
  ...
  it('merge safeguard: removing Claim:/ACCID: from Settings still matches real letters', () => {
    ...
    const cfg = accInboxConfigFromSettings(['Sample.Manager@example.test'], ['approv']);
    const realSubject = 'Ms Fakey McTestface - Claim:90000000001 ACCID:VEND-FAKE001';
    expect(
      isAccInboxCandidate(
        { sender: 'Sample.Manager@example.test', subject: realSubject, attachmentExt: '.pdf' },
        cfg,
      ),
    ).toBe(true);
  ...
  it('editable sender allowlist narrows matching for the strict candidate (still merged for subjects)', () => {
    const cfg = accInboxConfigFromSettings(['Sample.Manager@example.test'], []);
    expect(cfg.senderAllowlist).toEqual(['Sample.Manager@example.test']);
    const realSubject = 'Ms Fakey McTestface - Claim:90000000001 ACCID:VEND-FAKE001';
    expect(
      isAccInboxCandidate(
        { sender: 'Sample.Approver@example.test', subject: realSubject, attachmentExt: '.pdf' },
        cfg,
      ),
    ).toBe(false);
    expect(
...
describe('accInboxFilters — saved-file display rule (capture rule change)', () => {
  ...
  it('respects a narrowed sender allowlist from settings (but never subject)', () => {
    ...
    expect(
      isSavedAccInboxRow({ sender: 'Sample.Approver@example.test', attachmentExt: '.pdf' }, cfg),
    ).toBe(false);
    // The narrowed sender with a name-only subject is still shown.
    expect(isSavedAccInboxRow({ sender: 'Sample.Manager@example.test', attachmentExt: '.pdf' }, cfg)).toBe(
      true,
    );
  });
});
