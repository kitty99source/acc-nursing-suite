...
import type { StagingItem, StagingItemStatus } from './stagingTypes';
...
function isStagingIngressDuplicate(
  ...
  // 'deferred' rows are still awaiting a decision (they've moved to the
  // Review Queue's "Deferred" tab, not resolved), so their underlying sidecar
  // must keep being treated as already-present. Otherwise a re-import (e.g.
  // after an app restart resets the in-session seen-sidecar cache) would
...
async function cacheSidecarBytes(sc: StagingSidecar): Promise<void> {
  ...
  await saveLetterBlob(sc.item.sourceHash, blob);
}
...
const defaultEnricher: StagingEnricher = async (item) => {
  ...
  const preview = (await loadLetterParse(item.sourceHash)) as
    | { patientName?: string; claimNumber?: string }
    | undefined;
